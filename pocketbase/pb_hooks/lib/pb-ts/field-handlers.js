...
function textFieldSchema({ name, hidden, min, max, pattern, autogeneratePattern, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
...
function passwordFieldSchema({ name, hidden, min, max, pattern, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const typeDef = `${name}: string`;
    if (!includeDocs)
        return [typeDef, ''];
    const rows = [
...
function editorFieldSchema({ name, hidden, maxSize, convertURLs, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const typeDef = `${name}: string`;
    if (!includeDocs)
        return [typeDef, ''];
    const rows = [
...
function numberFieldSchema({ name, hidden, min, max, onlyInt, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
...
function boolFieldSchema({ name, hidden, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
...
function emailFieldSchema({ name, hidden, exceptDomains, onlyDomains, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const typeDef = `${name}: string`;
    if (!includeDocs)
        return [typeDef, ''];
    const rows = [
...
function urlFieldSchema({ name, hidden, exceptDomains, onlyDomains, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const typeDef = `${name}: string`;
    if (!includeDocs)
        return [typeDef, ''];
    const rows = [
...
function dateFieldSchema({ name, hidden, min, max, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const typeDef = `${name}: string`;
    if (!includeDocs)
        return [typeDef, ''];
...
function autodateFieldSchema({ name, hidden, onCreate, onUpdate }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const typeDef = `${name}: string`;
    if (!includeDocs)
        return [typeDef, ''];
    const docs = (0, utils_1.generateMDTable)([
...
function selectFieldSchema({ name, hidden, values, maxSelect, required, isMultiple }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
...
function fileFieldSchema({ name, hidden, maxSize, maxSelect, mimeTypes, thumbs, protected: $protected, required, isMultiple, }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const multiple = isMultiple();
...
function relationFieldSchema({ name, hidden, collectionId, collectionName, cascadeDelete, minSelect, maxSelect, required, isMultiple, }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
    const multiple = isMultiple();
    const typeDef = `${name}: ${multiple ? (required ? `[string, ...string[]]` : `string[]`) : 'string'}`;
    if (!includeDocs)
        return [typeDef, ''];
...
function jsonFieldSchema({ name, hidden, maxSize, required, override }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
...
function geoPointSchema({ name, hidden, required }, includeDocs = config_json_1.default.tsSchema.includeDocs) {
